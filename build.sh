...
# Also install node-level dependencies if needed
if [[ ! -d node/node_modules || "$*" == *--install* ]]; then
  echo "Installing node-level dependencies…"
  (cd node && npm install --legacy-peer-deps)
fi
