...
describe("A2A Protocol Endpoints", () => {
  ...
  before(async () => {
    ...
    await db.none(
      'INSERT INTO "user" (id, organization_id, email) VALUES ($(id), $(organizationId), $(email))',
      testUser,
    );
    await db.none(
      "INSERT INTO api_key (id, organization_id, user_id, key_hash, name) VALUES ($(id), $(organizationId), $(userId), $(keyHash), $(name))",
      testApiKey,
    );
  });
  ...
  describe("Core A2A Methods", () => {
    ...
    describe("tasks/get", () => {
      it("should retrieve task status", async () => {
        // First create a task
        const createResponse = await makeA2ARequest(
          "",
          "message/send",
          {
            message: {
        ...
        // Get task status
        const statusResponse = await makeA2ARequest(
          "",
          "tasks/get",
          {
            id: taskId,
    ...
    describe("tasks/cancel", () => {
      it("should cancel a running task", async () => {
        ...
        // Cancel the task
        const cancelResponse = await makeA2ARequest(
          "",
          "tasks/cancel",
          {
            id: taskId,
        ...
        expect(cancelResponse.body.result.status.state).to.be.oneOf([
          "canceled",
          "canceling",
        ]);
      });
  ...
  describe("A2A Protocol Compliance", () => {
    ...
    it("should return proper error codes", async () => {
      ...
      // Method not found
      const methodNotFoundResponse = await makeA2ARequest(
        "",
        "non/existent/method",
        {},
        {
      ...
      // Invalid params
      const invalidParamsResponse = await makeA2ARequest(
        "",
        "message/send",
        {
  ...
  describe("Discovery Endpoints", () => {
    it("should provide agent card at /.well-known/agent.json", async () => {
      const response = await fetch(
        `${TEST_CONFIG.shaman.a2a.publicUrl}/.well-known/agent.json`,
      );
    ...
    it("should list available agents at /.well-known/a2a/agents", async () => {
      const response = await fetch(
        `${TEST_CONFIG.shaman.a2a.publicUrl}/.well-known/a2a/agents`,
        {
          headers: {
            "x-api-key": testApiKey.key,
          },
  ...
  describe("Internal vs Public Server Behavior", () => {
    ...
    it("should expose all agents on internal server", async () => {
      ...
      const response = await fetch(
        `${TEST_CONFIG.shaman.a2a.internalUrl}/.well-known/a2a/agents`,
        {
          headers: {
