...
import {
  ...
  updateGitAgent,
  deleteGitAgent,
} from "./domain/git-agent/index.js";
...
export async function updateGitAgents(
  ...
  const agentsWithHashes = await parseAgentFilesWithCommitHashes(
    agentFiles,
    localPath,
    branch,
  );
  ...
  for (const agentData of agentsWithHashes) {
    ...
    const existing = existingAgents.find(
      (e) => e.filePath === agentData.filePath,
    );

    if (existing) {
      // Only update if the file's commit hash has changed
      if (
        existing.lastModifiedCommitHash !== agentData.lastModifiedCommitHash
      ) {
        // Updating agent - file changed
        const updated = {
          ...existing,
          ...agentData,
          agentRepositoryId: repository.id,
        };
...
export async function discoverAgentsFromBranch(
  repoUrl: string,
  branch: string = "main",
): Promise<
...
export async function discoverAgentsFromDefaultBranch(
  repoUrl: string,
): Promise<
  Omit<GitAgent, "id" | "agentRepositoryId" | "createdAt" | "updatedAt">[]
> {
...
async function ensureLocalRepo(
  ...
  if (!fs.existsSync(localPath)) {
    ...
    await execAsync(
      `git clone --single-branch --branch ${branch} ${repoUrl} ${localPath}`,
    );
  } else {
...
async function _parseAgentFiles(
  files: string[],
): Promise<
  Omit<GitAgent, "id" | "agentRepositoryId" | "createdAt" | "updatedAt">[]
> {
  const agents: Omit<
    GitAgent,
    "id" | "agentRepositoryId" | "createdAt" | "updatedAt"
  >[] = [];
  ...
  for (const file of files) {
    ...
    agents.push({
      ...
      providers:
        (parsed.data.providers as Record<string, unknown> | null) || null,
      mcpServers:
        (parsed.data.mcpServers as Record<string, unknown> | null) || null,
...
async function parseAgentFilesWithCommitHashes(
  files: string[],
  repoPath: string,
  branch: string = "main",
): Promise<
