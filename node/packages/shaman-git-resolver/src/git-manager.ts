...
export async function cloneOrPull(gitUrl: string, branch: string, repoName: string): Promise<string> {
  ...
  try {
    ...
    if (await fs.pathExists(gitDir)) {
      ...
      await execAsync(`git clone --single-branch --branch ${branch} --depth 1 ${gitUrl} ${dir}`);
...
export async function getLatestCommitHash(repoPath: string, branch: string = 'main'): Promise<string> {
  try {
...
export async function getFileCommitHash(repoPath: string, filePath: string, branch: string = 'main'): Promise<string | null> {
  try {
