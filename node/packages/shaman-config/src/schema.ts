...
import type { ShamanConfig, ValidationResult, DatabaseConfig, A2AConfig } from './types.js';
...
export function validateConfig(config: unknown): ValidationResult {
  ...
  if (cfg.llm && typeof cfg.llm === 'object') {
    ...
    if (llm.anthropic && typeof llm.anthropic === 'object') {
      ...
      if (anthropic.apiKey !== undefined && typeof anthropic.apiKey !== 'string') {
  ...
  if (cfg.a2a && typeof cfg.a2a === 'object') {
    ...
    if (a2a.authentication && typeof a2a.authentication === 'object') {
      ...
      if (!auth.type || !['none', 'bearer', 'basic'].includes(auth.type as string)) {
        errors.push('a2a.authentication.type must be one of: none, bearer, basic');
      }
    }
  }
  ...
  if (cfg.agents && typeof cfg.agents === 'object') {
    ...
    if (agents.syncInterval !== undefined && typeof agents.syncInterval !== 'number') {
...
export function mergeConfigs(
  ...
  if (override.a2a || base.a2a) {
    ...
    if (override.a2a?.authentication || base.a2a?.authentication) {
      const authType = override.a2a?.authentication?.type || base.a2a?.authentication?.type;
      if (authType) {
    ...
    if (override.a2a?.rateLimiting || base.a2a?.rateLimiting) {
      const enabled = override.a2a?.rateLimiting?.enabled ?? base.a2a?.rateLimiting?.enabled;
