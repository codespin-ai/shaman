...
export class JsonRpcHandler {
  ...
  private async handleBatch(
    ...
    return responses.filter(response => response !== undefined) as JsonRpcBatchResponse;
  }
  ...
  private parseRequest(body: unknown): Result<JsonRpcRequest | JsonRpcBatchRequest, string> {
    try {
      if (typeof body !== 'object' || body === null) {
        return { success: false, error: 'Request body must be an object or array' };
      }

      return { success: true, data: body as JsonRpcRequest | JsonRpcBatchRequest };
    } catch (error) {
  ...
  private validateRequest(request: JsonRpcRequest): Result<void, string> {
    ...
    if (request.method.startsWith('rpc.')) {
      return { success: false, error: 'Method names should not start with "rpc."' };
    }
  ...
  private createSuccessResponse<T>(id: string | number | null, result: T): JsonRpcSuccessResponse<T> {
    return {
      jsonrpc: '2.0',
      result,
      id,
  ...
  private createErrorResponse(
    ...
    const error: JsonRpcError = data !== undefined
      ? { code, message, data }
      : { code, message };
