...
export function createCallAgentTool(
  ...
  return {
    name: "call_agent",
    description:
      "Call another agent and get its response. Enables agent collaboration and delegation.",
    inputSchema: z.object({
      agent: z
        .string()
        .describe(
          'Name of the agent to call (e.g., "InventoryAgent" or "FraudDetectionAgent@git+https://github.com/org/agents.git#main")',
        ),
      message: z
        .string()
        .describe("The message or request to send to the agent"),
      contextData: z
        .record(z.unknown())
        .optional()
        .describe("Optional context data to pass to the agent"),
      async: z
        .boolean()
        .optional()
        .default(false)
        .describe(
          "Whether to wait for completion (false) or return immediately with a task ID (true)",
        ),
    }),
    async execute(input) {
      const {
      ...
      if (!options.jwtToken && !context.jwtToken) {
        throw new Error(
          "No JWT token available for internal agent communication",
        );
      }
      ...
      const a2aClient = createA2AClient({
        baseUrl:
          options.internalA2AUrl ||
          process.env.INTERNAL_A2A_URL ||
          "http://localhost:5001",
...
function extractAgentResponse(task: any): any {
  ...
  if (task.artifacts && Array.isArray(task.artifacts)) {
    const jsonArtifact = task.artifacts.find(
      (a: any) =>
